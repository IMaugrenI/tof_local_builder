...
"$VENV_DIR/bin/pip" install --disable-pip-version-check --no-input -r "$ROOT_DIR/services/repo_bridge/requirements.txt" >/dev/null

export SANDBOX_ROOT="$ABS_SANDBOX_PATH"
export ALLOW_ORIGINS="${ALLOW_ORIGINS:-http://localhost:3000,http://127.0.0.1:3000}"

(
  cd "$ROOT_DIR/services/repo_bridge"
  nohup "$VENV_DIR/bin/python" -m uvicorn host_main:app --host "$BRIDGE_HOST" --port "$BRIDGE_PORT" >"$LOG_FILE" 2>&1 &
  echo $! >"$PID_FILE"
)
